...
class HopperEnvironment(Environment):
    """
    Brax Hopper environment for NEAT training.

    The agent controls a 2D hopper to move forward by applying
    ...
    def evaluate(self, net, gym_env: gym.Env) -> float:
        """
        Run one Hopper episode and return the fitness.
        ...
        Args:
            ...
            gym_env: Initialized Hopper environment
    ...
    def get_normalization_range(self) -> Tuple[float, float]:
        """
        Return normalization range for Hopper fitness.

        Hopper-v4 typically yields rewards in the range of 0 to ~3000+.
